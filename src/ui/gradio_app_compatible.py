"""Fixed Gradio UI for Travel Assistant - Compatible Version"""
...
with gr.Blocks(title="🌍 Travel Assistant AI") as demo:
    # Header
    gr.HTML("""
    <div style="text-align: center; padding: 30px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 15px; margin-bottom: 20px; color: white; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.3);">
    ...
    with gr.Column():
        # Chatbot component
        chatbot = gr.Chatbot(
            value=[], height=500, show_label=False, show_copy_button=True
        )
...
def launch_ui(server_port: int = 7860, share: bool = False):
    ...
    logger.info(f"Launching fixed Travel Assistant UI on port {server_port}...")
    demo.launch(
